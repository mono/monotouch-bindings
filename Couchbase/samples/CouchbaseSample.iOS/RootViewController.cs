  ...
  public partial class RootViewController : UIViewController
  {
    ...
    public override void ObserveValue (NSString keyPath, NSObject ofObject, NSDictionary change, IntPtr context)
    {
      ...
      if (DoneQuery.Rows.Count == 0){
        ...
        var row = DoneQuery.Rows.RowAtIndex (0);
        var doc = row.Value as NSDictionary;

        val = String.Format ("{0}: {1}\t", doc.ValueForKey ((NSString)"Label"), doc.ValueForKey ((NSString)"Count"));
      }
      label.Text = val;
    }
    #endregion
    ...
    void AddNewItem (object sender, EventArgs args)
    {
      ...
      var vals = NSDictionary.FromObjectsAndKeys (
        ...
        new NSObject[] {
        new NSString (DocumentDisplayPropertyName),
        new NSString (CheckboxPropertyName),
        new NSString (CreationDatePropertyName)
      }
      );
