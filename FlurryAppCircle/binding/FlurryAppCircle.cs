...
namespace FlurryAppCircle
{
	...
	interface FlurryOffer {
		...
		[Export ("appIcon", ArgumentSemantic.Retain)]
		UIImage AppIcon { get; set;  }

		[Export ("referralUrl", ArgumentSemantic.Retain)]
		string ReferralUrl { get; set;  }

		[Export ("appPrice", ArgumentSemantic.Retain)]
		NSNumber AppPrice { get; set;  }

		[Export ("appDescription", ArgumentSemantic.Retain)]
